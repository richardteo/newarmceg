...
sheet = client.open('Seletar SFT Records').worksheet('TPTHubEast')
...
def submit(update: Update, _: CallbackContext):
    ...
    if update.message.text == 'Yes' or update.message.text == 'yes':
        ...
        bot.sendMessage(chat_id = -1001279862226, text = (
                                                        '🏃New Sign In Entry🏃' + '\n\n'
                                                        'Date: ' + Date + '\n'
                                                        'Name: ' + Person + '\n'
                                                        'Activity: ' + Activity + '\n'
...
def main() -> None:
    ...
    updater.start_webhook(listen="0.0.0.0",
                          port=PORT,
                          url_path=TOKEN,webhook_url= 'https://infinite-sea-19023.herokuapp.com/' + TOKEN)
