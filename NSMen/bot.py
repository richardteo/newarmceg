...
sheet = client.open('Seletar SFT Records').worksheet('SeletarNSMen')
    ...
    # Ask for password
        ...
        if (check_in_time >= morning_start and check_in_time <= night_end):
            ...
            if userID not in userID_database:
                ...
                'Hi! My name is Seletar NSMen Bot.\n'
...
def submit(update: Update, _: CallbackContext):
    ...
    if update.message.text == 'Yes' or update.message.text == 'yes':
        ...
        bot.sendMessage(chat_id = -1001488591469, text = (
                                                        '🏃New Sign In Entry🏃' + '\n\n'
                                                        'Date: ' + Date + '\n'
                                                        'Name: ' + Person + '\n'
                                                        'Activity: ' + Activity + '\n'
...
def main() -> None:
    ...
    updater.start_webhook(listen="0.0.0.0",
                          port=PORT,
                          url_path=TOKEN,webhook_url= 'https://protected-mesa-63029.herokuapp.com/' + TOKEN)
